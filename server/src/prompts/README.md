...
Information state has three deliberately separate layers. `byCandidate.*.revealedIds` records human-surfaced traits, `aiSurfacedIds` records Alex-surfaced traits for pooling DV and repetition prevention, and `humanConfirmedIds` records the human-grounded discussion board used by depth, summary eligibility, and intervention judgment. Summary, closing, complete visible-board answers, and preference use the same deduplicated visible-board union of human and Alex disclosures. Existing sessions without `humanConfirmedIds` fall back to their legacy human-surfaced set.
