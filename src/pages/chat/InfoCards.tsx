    ...
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="w-full flex items-center justify-between rounded-lg bg-muted/50 px-4 py-3 hover:bg-muted transition-colors">
        <div className="flex items-center gap-3">
          <span className={cn("w-8 h-8 rounded-lg flex items-center justify-center font-mono font-bold text-sm", candidateColors[candidate])}>
            {candidate}
          </span>
      ...
      <CollapsibleContent>
        <div className="space-y-2 pt-2 pl-1">
          {cards.map((card, i) => <InfoCardItem key={i} card={card} />)}
        </div>
      </CollapsibleContent>
    </Collapsible>
  );
...
const InfoCards = () => {
  ...
  const grouped = (["A", "B", "C", "D"] as Candidate[]).reduce((acc, c) => {
